...
// Suck-a-Coin: optional setting to unlock wallet for block minting only;
...
// Suck-a-Coin: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // Suck-a-Coin: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            for (;;)
            {
                ...
                // Suck-a-Coin: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    if (!GetBoolArg("-avatar")) // Suck-a-Coin: not avatar mode
                    {
...
// Suck-a-Coin: create coin stake transaction
...
// Suck-a-Coin: check 'spent' consistency between wallet and txindex
// Suck-a-Coin: fix wallet spent state according to txindex
...
// Suck-a-Coin: disable transaction (only for coinstake)

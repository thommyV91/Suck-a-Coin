...
const string strMessageMagic = "Suck-a-Coin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // Suck-a-Coin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
...
bool CTransaction::CheckTransaction() const
{
    ...
    for (int i = 0; i < vout.size(); i++)
    {
        ...
        // Suck-a-Coin: enforce minimum output amount
                        ...
                        bool* pfMissingInputs)
{
    ...
    // Suck-a-Coin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
// Suck-a-Coin: find block wanted by given orphan block
...
// Suck-a-Coin: miner's coin stake is rewarded based on coin age spent (coin-days)
...
// Suck-a-Coin: find last block index up to pindex
...
unsigned int static GetNextTargetRequired(const CBlockIndex* pindexLast, bool fProofOfStake)
{
    ...
    if (!fProofOfStake && pindexLast->nHeight >= 23 && pindexLast->nHeight < 120)
        ...
        // Suck-a-Coin: block stuck at 227
    if (!fProofOfStake && pindexLast->nHeight >= 277 && pindexLast->nHeight < 400)
        return bn227HashTarget.GetCompact();

    int64 nActualSpacing = pindexPrev->GetBlockTime() - pindexPrevPrev->GetBlockTime();

    // Suck-a-Coin: target change every block
    // Suck-a-Coin: retarget with exponential moving toward target spacing
    CBigNum bnNew;
...
void static InvalidChainFound(CBlockIndex* pindexNew)
{
    ...
    // Suck-a-Coin: should not enter safe mode for longer invalid chain
}
                                 ...
                                 bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // Suck-a-Coin: check transaction timestamp
        ...
        if (IsCoinStake())
        {
            // Suck-a-Coin: coin stake tx earns reward instead of paying fee
            uint64 nCoinAge;
        ...
        else
        {
            ...
            // Suck-a-Coin: enforce transaction fees for every block
            if (nTxFee < GetMinFee())
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Suck-a-Coin: cleanup wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // Suck-a-Coin: track money supply and mint amount info
    ...
    // Suck-a-Coin: fees are not collected by miners as in bitcoin
    // Suck-a-Coin: fees are destroyed to compensate the entire network
    if (fDebug && GetBoolArg("-printcreation"))
...
// Suck-a-Coin: total coin age spent in transaction, in the unit of coin-days.
...
// Suck-a-Coin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos)
{
    ...
    // Suck-a-Coin: compute chain trust score
    pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

    // Suck-a-Coin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // Suck-a-Coin: record proof-of-stake hash value
    ...
    // Suck-a-Coin: compute stake modifier
...
bool CBlock::CheckBlock(int64 nHeight) const
{
    ...
    // Suck-a-Coin: only the second transaction can be the optional coinstake
    for (int i = 2; i < vtx.size(); i++)
        if (vtx[i].IsCoinStake())
            return DoS(100, error("CheckBlock() : coinstake in wrong position"));

    // Suck-a-Coin: coinbase output should be empty if proof-of-stake block
    ...
    // Suck-a-Coin: check block signature
...
bool CBlock::AcceptBlock()
{
    ...
    // Suck-a-Coin: check that the block satisfies synchronized checkpoint
    ...
    // Suck-a-Coin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // Suck-a-Coin: check proof-of-stake
    ...
    // Suck-a-Coin: verify hash target and signature of coinstake tx
    if (pblock->IsProofOfStake())
    {
        ...
        if (!CheckProofOfStake(pblock->vtx[1], pblock->nBits, hashProofOfStake))
        {
            ...
            // Suck-a-Coin: ask for missing blocks
            if (pfrom)
    ...
    // Suck-a-Coin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // Suck-a-Coin: getblocks may not obtain the ancestor block rejected
    ...
    // Suck-a-Coin: if responsible for sync-checkpoint send it
...
// Suck-a-Coin: entropy bit for stake modifier if chosen by modifier
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        ThreadSafeMessageBox(strMessage, "Suck-a-Coin", wxOK | wxICON_EXCLAMATION | wxMODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    printf("%s Network: genesis=0x%s nBitsLimit=0x%08x nBitsInitial=0x%08x nStakeMinAge=%d nCoinbaseMaturity=%d nModifierInterval=%d\n",
           fTestNet? "Test" : "Suck-a-Coin", hashGenesisBlock.ToString().substr(0, 20).c_str(), bnProofOfWorkLimit.GetCompact(), bnInitialHashTarget.GetCompact(), nStakeMinAge, nCoinbaseMaturity, nModifierInterval);
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Suck-a-Coin: initialize synchronized checkpoint
        if (!Checkpoints::WriteSyncCheckpoint(hashGenesisBlock))
            return error("LoadBlockIndex() : failed to init sync checkpoint");
    }

    // Suck-a-Coin: if checkpoint master key changed must reset sync-checkpoint
    {
        CTxDB txdb;
        string strPubKey = "";
...
string GetWarnings(string strFor)
{
    ...
    // Suck-a-Coin: wallet lock warning for minting
    if (strMintWarning != "")
    {
        nPriority = 0;
    ...
    // Suck-a-Coin: should not enter safe mode for longer invalid chain
    // Suck-a-Coin: if sync-checkpoint is too old do not enter safe mode
    ...
    // Suck-a-Coin: if detected invalid checkpoint enter safe mode
    ...
    // Alerts
    {
        ...
        BOOST_FOREACH(PAIRTYPE(const uint256, CAlert)& item, mapAlerts)
        {
            ...
            if (alert.AppliesToMe() && alert.nPriority > nPriority)
            {
                ...
                if (nPriority > 1000)
                    strRPC = strStatusBar;  // Suck-a-Coin: safe mode for high alert
            }
        }
    }
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // Suck-a-Coin: record my external IP reported by peer
        ...
        // Suck-a-Coin: relay sync-checkpoint
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    if (inv.hash == pfrom->hashContinue)
                    {
                        ...
                        // Suck-a-Coin: best block is always the reference block to prevent
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // Suck-a-Coin: tell downloading node about the latest block if it's
...
CBlock* CreateNewBlock(CReserveKey& reservekey, CWallet* pwallet, bool fProofOfStake)
{
    ...
    // Suck-a-Coin: if coinstake available add coinstake tx
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // Suck-a-Coin: simplify transaction fee - allow free = false
...
void BitcoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateBitcoins || fProofOfStake)
    {
        ...
        if (fProofOfStake)
        {
            // Suck-a-Coin: if proof-of-stake block found then process block
            if (pblock->IsProofOfStake())
            {
